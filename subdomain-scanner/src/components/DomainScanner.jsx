...
const DomainScanner = () => {
  ...
  const fetchData = async (url, setResultState, resultType) => {
    ...
    if (validationError) return setResultState(validationError);
    ...
    try {
      ...
      if (data.error) {
        ...
        setResultState(data.malwareAnalysis);
      } else {
        const resultData = {
    ...
    <div className="scanner-container">
      ...
      <div className="input-section">
        ...
        <div className="button-group">
          <button className="scan-button" onClick={() => fetchData("http://127.0.0.1:5000/api/subdomains", setResults, "subdomains")}>
            🔍 Subdomain Scan
          </button>
          <button className="scan-button malware-button" onClick={() => fetchData("http://127.0.0.1:5000/api/malware", setMalwareResults, "malware analysis")}>
            🛡️ Malware Analysis
          </button>
          <button className="scan-button whois-button" onClick={() => fetchData("http://127.0.0.1:5000/api/whois", setWhoisResults, "WHOIS data")}>
            🌐 WHOIS Data
          </button>
          <button className="scan-button ssl-button" onClick={() => fetchData("http://127.0.0.1:5000/api/ssl", setSslResults, "SSL/TLS details")}>
            🔑 SSL/TLS Check
          </button>
          <button 
